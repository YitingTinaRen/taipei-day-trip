...
class attraction:
    ...
    def load_attractions(page, keyword):
        ...
        try:
            if keyword:
                attractions = model.db.search_by_keyword(
                    keyword, page, NumInOnePage)
            else:
            ...
            if len(attractions) == 0:  # if serch result is empty
                return jsonify(
                    {"error": True, "message": "Empty result."}), 500
        ...
        if len(attractions) == NumInOnePage + \
                1:  # check if there will be at least one more data in the next page
            return jsonify(
                dict(data=attractions[0:NumInOnePage], nextPage=page + 1)), 200

        else:
            return jsonify(
                dict(data=attractions[0:NumInOnePage], nextPage=None)), 200

    def load_by_id(attractionId):
        try:
            attractions = model.db.search_by_id(attractionId)
            if len(attractions) == 0:
                # if serch result is empty
                return jsonify(
                    {"error": True, "message": "ID does not exist."}), 400
        except BaseException:
            return jsonify(
                {"error": True, "message": "Server internal error."}), 500
