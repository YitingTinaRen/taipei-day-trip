...
class orders:
    def make_order(token, data):
        ...
        if "error" in token_info:
            return jsonify(
                {"error": True, "message": token_info["message"]}), 403

        if not token_info["data"]["name"] == data["contact"]["name"] or not token_info["data"]["email"] == data["contact"]["email"]:
            return jsonify(
                {"error": True, "message": "User name and email conflict with contact information."}), 400
        elif not re.match(r'09\d{8}', data["contact"]["phone"]):
            return jsonify(
                {"error": True, "message": "Wrong phone format."}), 400
        ...
        if model.db.record_order(
                res,
                booking_id,
                data["contact"]["phone"]) and not res["status"] and model.db.confirm_booking(
                token_info["data"]["id"]):
    ...
    def delete_order(token, order_num):
        ...
        if result and refundResult["status"] == 0:
            ...
            return jsonify(
                {"error": True, "message": refundResult["msg"]}), 500
        elif not result:
